...
def load_data(batch_size=1, suffix=''):
    loc_train = np.load('nri/data/loc_train' + suffix + '.npy')
    vel_train = np.load('nri/data/vel_train' + suffix + '.npy')
    edges_train = np.load('nri/data/edges_train' + suffix + '.npy')

    loc_valid = np.load('nri/data/loc_valid' + suffix + '.npy')
    vel_valid = np.load('nri/data/vel_valid' + suffix + '.npy')
    edges_valid = np.load('nri/data/edges_valid' + suffix + '.npy')

    loc_test = np.load('nri/data/loc_test' + suffix + '.npy')
    vel_test = np.load('nri/data/vel_test' + suffix + '.npy')
    edges_test = np.load('nri/data/edges_test' + suffix + '.npy')
...
def edge_accuracy(preds0, target):
    _, preds = preds0.max(-1)
    
    preds = torch.median(preds, axis=0).values
    preds = preds.repeat(target.shape[0], 1)    
