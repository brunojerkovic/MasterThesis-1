...
import utils
from models.ngc_noise.sourcecode.cmlp import cMLP, train_model_ista, test_model
from models.ngc_noise.sourcecode.clstm import cLSTM, train_model_ista as s
from models.ngc_noise.data_cleaner import clean_data
from models.model import Model
from result_saver import ResultSaver

from models.ngc_srsv.neural_SVM.main_NSVM import main_nsvm
...
class NGC(Model):
    ...
    def _algorithm(self, series, coef_mat, edges) -> tuple:
        ...
        train_loss_list, val_loss_list = train_function(
            cmlp,
            X_train,
            X_valid, # context=10
            lam=self.lam,
        ...
        # u proslim linijama koda ti ovo radis: 1 / 6239 * ((y_train_t1 - X_train_np) - np.mean(y_train_t1 - X_train_np)).T @ ((y_train_t1 - X_train_np) - np.mean(y_train_t1 - X_train_np))
        # Tocniji je MLE nacin od dolje (technique 2)
        print("ESTIMATED COV (technique 1)", epsilon_cov_est)

        # EPSILON COV. MLE
        epsilon_cov_est = 1 / y_train_t1.shape[0] * (y_train_t1 - X_train_np).T @ (y_train_t1 - X_train_np)
        print(f"ESTIMATED COV (technique 2)", epsilon_cov_est)
        ...
        # GET MEAN AND VAR
        print("MEAN AND VAR\n")
        print([(net.mean, net.var) for net in cmlp.networks])
        exit(0)
