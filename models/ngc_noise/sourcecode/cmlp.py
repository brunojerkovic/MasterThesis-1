...
class MLP(nn.Module):
    def __init__(self, num_series, lag, hidden, activation, model_choice, logvar):
        ...
        # Set up network.
        layer = nn.Conv1d(num_series, hidden[0], lag) if model_choice == 'ngc' else nn.Conv1d(num_series, 1, lag)
        modules = [layer]
...
class cMLP(nn.Module):
    def __init__(self, num_series, lag, hidden, activation='relu', model_choice=None):
        ...
        self.logvar = nn.Parameter(torch.rand((num_series, num_series), device='cuda'), requires_grad=True) # TODO: cuda warning here
...
def train_model_gista(cmlp, X, lam, lam_ridge, lr, penalty, max_iter,
    ...
    Args:
      clstm: clstm model.
      X: tensor of datagenerators, shape (batch, T, p).
...
def train_model_ista(cmlp, X, lr, max_iter, lam=0, lam_ridge=0, penalty='H',
    ...
    eps = torch.randn((p, N-p)).cuda() # TODO: cuda warning here
    loss = sum([loss_fn(cmlp.networks[i](X[:, :-1], eps, i), X[:, lag:, i:i+1]) for i in range(p)])
